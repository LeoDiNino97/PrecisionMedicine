...
install.packages("GGally",force = TRUE)
...
cgsubcolnames(rna.expr.data.N) <- gsub("\\.", "-", substr(colnames(rna.expr.data.N), 1,12))
...
#Data Normalization:
all(rownames(expr.C.men) == rownames(expr.N.men))

full.data.men <- cbind(expr.C.men, expr.N.men)
full.data.women <- cbind(expr.C.women, expr.N.women)
meta.men <- rep("normal",(2*dim(full.data.men)[2]))
meta.men[1:length(meta.men)] <- "cancer"
meta.men <- data.frame(meta.men)
colonne <- paste0(nomi.colonne, nomi.colonne)
colonne_stacked <- c(colonne, colonne)
rownames(meta.men) <- colonne_stacked


meta.women <- rep("normal",(2*dim(full.data.women)[2]))
meta.women <- rep("normal",(2*dim(full.data.women)[2]))
meta.women[1:length(meta.women)/2] <- "cancer"
data.frame(meta.women)



colnames(meta.women)[1]<- "condition"
...
n.men <- dim(expr.C.men)[2] #same value for non cancer patients
Z.men <- (cor.mat.C.men - cor.mat.N.men)/((sqrt(1/(n.men - 3)))+(sqrt(1/(n.men - 3))))
adj.mat.men <- 1 * (abs(Z.men) > Z)

# Women graph
# Sample size of men between condition C and condition N is the same by design
n.women <- dim(expr.C.women)[2]
Z.women <- (cor.mat.C.women - cor.mat.N.women)/((sqrt(1/(n.women - 3)))+(sqrt(1/(n.women - 3))))
...
network.edgecount(DEnet.men) # 217
network.density(DEnet.men) #0.001031977
...
#x.men <- cbind(1, x.men[2:length(x.men)])  # Start from 2 to match original subset
...
network.edgecount(DEnet.women) # 265
network.density(DEnet.women) #0.001260248
...
#x.women <- cbind(1, x.women[2:length(x.women)])  # Start from 2 to match original subset
...
#Evaluate the overlap between the sets of female-hubs and male-hubs.
# Overlapping hubs
...
x.C <- cbind(1, x.C[2:length(x.C)])
model.lm.C <- glm.fit(x.C, y.C[2:length(y.C)])
beta.c <- model.lm.C$coefficients
plot(x.C[,2], y.C[2:length(y.C)], xlab = "log(degree)", ylab = "log(count)", main = " ",pch = 16, col = "lightblue")

# Add the fitted line
abline(a = beta.c[1], b = beta.c[2], col = 'red', lty = 'dotted')
...
# Optional tasks ----------------------------------------------------------
#5.1)
#Co-expr. Network female vs. male onto Normal tissue
#compute corr matrices
cor.mat.N.men <- corr.test(t(expr.N.men), use = "pairwise", 
                           method = "pearson", adjust="fdr", ci=FALSE)
cor.mat.N.women <- corr.test(t(expr.N.women), use = "pairwise", 
...
#5.2) --------------------------------------------------------------------------
#extract the two subnetworks (network with only positive links and network with only negative links)
DEnet.C.bonus <- as.network(adj.mat.C, directed=TRUE)
DEnet.C.bonus
